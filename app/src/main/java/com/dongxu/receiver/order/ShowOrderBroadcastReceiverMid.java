 ...
 */
public class ShowOrderBroadcastReceiverMid extends BroadcastReceiver {

    public static final String TAG = "dx";

    @Override
    public void onReceive(Context context, Intent intent) {
        Log.i(TAG, "------ onReceive order mid ------");
        ...
        if (ReceiverCommon.RECEIVER_ORDER_ACTION.equals(action)) {
            ...
            Log.i(TAG, "------ onReceive order mid ------ action --- data: " + data);

            //拦截广播，并修改广播内容
            setResultData("乡长说：每人发送一个5元红包！！！");
